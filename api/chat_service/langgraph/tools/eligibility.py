"""정책 자격 진단(추천) 공통 tool — 도메인 검색 노드가 공유한다.

이전엔 housing_search_node에만 있던 자격 진단 로직(나이/지역/혼인 검사 → 자격 적합순 정렬 +
✅/❌/❓ 라벨링)을 여기로 모아, employment·education 검색 노드도 동일하게 추천(자격 적합 우선
정렬·라벨)을 적용할 수 있게 한다.

- LLM 미사용, 정규식/키워드 기반 (검색 시점에 가볍게 동작).
- user_profile이 비어있으면(=게스트) 진단을 스킵한다(None 반환).
- 검사 항목은 도메인 무관한 공통 메타(나이/지역/혼인)만 사용하므로 전 분야에 적용 가능하다.
  (소득·취업·학력 등 도메인 특화 검증은 welfare_agent.check_eligibility가 별도로 담당)
"""
