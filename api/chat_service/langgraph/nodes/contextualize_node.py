...
1) standalone_query: 이전 대화 맥락을 모두 반영한 독립형 질의로 다시 쓰세요.
   ...
   - 사용자가 새 메시지에서 본인 정보(학점·소득·나이 등 구체 수치)를 새로 언급했다면,
     지시대상을 정책명으로 구체화하더라도 그 수치는 **절대 생략하지 말고** standalone_query에
     그대로 포함하세요. (예: "평점 4.3이야, 성적우수 장학금 알려줘" →
     "평점 4.3인데 성적우수 장학금에 대한 자세한 정보를 알려줘." — 수치를 살려서 재작성)
2) referenced_indices: 사용자가 가리킨 [직전 정책]의 1-based 번호. 없으면 [].
   ...
   - 사용자가 "이거/그거/두번째/방금 그것"처럼 **지시대명사·순번으로만** 가리킬 때만 번호를 채우세요.
     [직전 정책] 목록에 없는 **새로운 정책명·주제를 구체적으로 언급**했다면(예: "성적우수 장학금",
     "청년내일저축계좌" 등 직전에 안 나온 이름), 그건 새 검색 대상이지 직전 정책을 가리키는 게
     아니므로 referenced_indices는 반드시 빈 리스트 []로 두세요. 본인 정보(학점 등)를 언급했다는
     이유만으로 직전 정책을 가리킨다고 추측하지 마세요.
